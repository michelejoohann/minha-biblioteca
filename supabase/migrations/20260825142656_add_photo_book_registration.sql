...
drop policy if exists "Household members can read book covers" on storage.objects;
drop policy if exists "Members can upload their book covers" on storage.objects;
drop policy if exists "Members can delete their book covers" on storage.objects;

create policy "Household members can read book covers"
...
using (
  ...
  and case
    ...
    then (select private.is_household_member(((storage.foldername(name))[1])::uuid))
    else false
  end
);

create policy "Members can upload their book covers"
...
create policy "Members can delete their book covers"
...
declare
  ...
  clean_cover_path text := nullif(btrim(p_cover_path), '');
  clean_genres text[];
begin
  ...
  if clean_cover_path is not null and (
    length(clean_cover_path) > 600
    or split_part(clean_cover_path, '/', 1) <> p_household_id::text
    or split_part(clean_cover_path, '/', 2) <> current_user_id::text
    or split_part(clean_cover_path, '/', 3) = ''
  ) then
    raise exception using errcode = '22023', message = 'Caminho da capa inválido.';
  end if;
